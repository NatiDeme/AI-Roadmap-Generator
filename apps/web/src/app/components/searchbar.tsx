...
import { useGenerateRoadmap } from "../services/query.service";
...
function Searchbar() {
  ...
  const handleSubmit = async(e: React.FormEvent) => {
    e.preventDefault();
    try {
      if(!goal || !currentSkill || hoursPerWeek === null) {
  ...
  return (
    <form className="flex flex-col gap-3 w-96 md:w-[500px] border-2 border-gray-300 rounded-2xl justify-between p-6" onSubmit={handleSubmit}>
      ...
      <input type="number" name="hoursPerWeek" placeholder="e.g 10 hours per week" className="w-full border-2 rounded-2xl h-14 p-2 border-gray-300 focus:ring-0 focus:outline-none" value={hoursPerWeek ?? ''} onChange={(e) => setHoursPerWeek(e.target.value === '' ? null : Number(e.target.value))} />
      
      <div className="flex justify-between items-center border-t-2 border-gray-100 py-2">
        <button
          type="submit"
          disabled={isPending}
          className="bg-black text-white rounded-full p-4 hover:bg-gray-700 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
        >
